...
class LatticeLSTM(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, word_drop, char_alphabet_size, char_emb_dim, pretrain_word_emb=None,
                 left2right=True, fix_char_emb=True, gpu=True, use_bias=True):
        super(LatticeLSTM, self).__init__()
        skip_direction = "forward" if left2right else "backward"
        print("build LatticeLSTM... ", skip_direction, ", Fix emb:", fix_char_emb, " gaz drop:", word_drop)
        self.gpu = gpu
        self.hidden_dim = hidden_dim
        self.char_emb = nn.Embedding(char_alphabet_size, char_emb_dim)
        if pretrain_word_emb is not None:
            print("load pretrain word emb...", pretrain_word_emb.shape)
            self.char_emb.weight.data.copy_(torch.from_numpy(pretrain_word_emb))
        else:
            self.char_emb.weight.data.copy_(torch.from_numpy(self.random_embedding(char_alphabet_size, char_emb_dim)))
        if fix_char_emb:
        ...
        self.word_rnn = WordLSTMCell(char_emb_dim, hidden_dim)
        ...
        for index in range(batch_size):
            ...
            for t in id_list:
                ...
                if skip_input_[t]:
                    ...
                    char_emb = self.char_emb(word_var)
                    char_emb = self.word_dropout(char_emb)
                    ct = self.word_rnn(char_emb, (hx_, cx_))
...
class MultiInputLSTMCell(nn.Module):
    ...
    def reset_parameters(self):
        ...
        Initialize parameters following the way proposed in the paper.
        """
        # UserWarning: nn.init.orthogonal is now deprecated in favor of nn.init.orthogonal_.init.orthogonal(self.weight_ih.data)
        init.orthogonal_(self.weight_ih.data)
